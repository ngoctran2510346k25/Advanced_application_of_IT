# 📚 Ôn thi Tin học văn phòng — 398 câu trắc nghiệm

> 🎯 Ứng dụng web luyện thi trắc nghiệm **Microsoft Word · Excel · Access 2010**, có đáp án, giải thích và mẹo khoanh.

![HTML](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![CSS](https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white)
![GitHub Pages](https://img.shields.io/badge/GitHub%20Pages-222?style=for-the-badge&logo=githubpages&logoColor=white)

---

## 👉 Vào học ngay

**[🔗 Mở ứng dụng ôn thi]([https://ngoctran2510346k25.github.io/-DCNTT-N-ng-cao/](https://ngoctran2510346k25.github.io/Advanced_application_of_IT/))**

*(Nhớ thay bằng link GitHub Pages thật của bạn)*

---

## ✨ Tính năng

- 🎯 **Luyện thông minh** — ưu tiên câu chưa thuộc & hay sai (lặp lại ngắt quãng)
- 📚 **Luyện toàn bộ theo thứ tự** — lần lượt 398 câu
- 🔀 **Trộn ngẫu nhiên** — xáo trộn toàn bộ câu hỏi
- 🔁 **Ôn lại câu sai & đánh dấu** — tập trung phần còn yếu
- 🔎 **Tra cứu** — tìm theo số câu hoặc từ khoá
- 📊 **Theo dõi tiến độ** — tự lưu câu đã thuộc, hiển thị % hoàn thành

## 📦 Nội dung

| Phần | Số câu |
|------|:------:|
| Microsoft Word | 137 |
| Microsoft Excel | 126 |
| Microsoft Access | 133 |
| Khác | 2 |
| **Tổng** | **398** |

## ⌨️ Phím tắt (trên máy tính)

- `1`–`4` hoặc `A`–`D` : chọn đáp án
- `Enter` : câu tiếp theo
- `S` : đánh dấu sao

---

<p align="center">Made with ☕ for ôn thi · Chúc bạn thi tốt! 🍀</p>
